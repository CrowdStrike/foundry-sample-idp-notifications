...
export class WorkflowsPage extends BasePage {
  ...
  /**
   * Verify that an action is available in the workflow builder
   */
  async verifyActionAvailable(actionName: string): Promise<boolean> {
    return this.withTiming(
      async () => {
        this.logger.info(`Verifying action available: ${actionName}`);

        // The "Add next" dialog should already be open
        // Click on the first action type button (flag icon)
        const actionButton = this.page.locator('button').filter({ hasText: 'Action' }).locator('..').locator('button').first();
        ...
        // Check if action appears in the results
        const actionElement = this.page.getByText(actionName, { exact: false });
        const isVisible = await actionElement.isVisible({ timeout: 30000 });

        if (isVisible) {
          this.logger.success(`Action found: ${actionName}`);
        } else {
          this.logger.error(`Action not found: ${actionName}`);
        }

        // Close the action picker
        await this.page.keyboard.press('Escape');

        return isVisible;
      },
      `Verify action available: ${actionName}`
    );
  }
}